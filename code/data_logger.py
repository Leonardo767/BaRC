...
class DataLogger:
    def __init__(self, col_names, filepath, data_dir, auto_save_every=4):
        ...
        self.iterable_columns = [
            col_name for col_name in col_names if col_name != 'timestamp'
        ]
    ...
    def add_rows(self, cols_values_dict, update_indices=list()):
        ...
        for col in cols_values_dict:
            if len(cols_values_dict[col]) < max_len:
                cols_values_dict[col].extend(
                    [cols_values_dict[col][-1]] *
                    (max_len - len(cols_values_dict[col])))
        ...
        for col in self.iterable_columns:
            if col in cols_values_dict:
                ...
                for itr in range(max_len):
                    self.internal_dict[col].append(self.indices_dict[col] +
                                                   itr)

            elif col in self.indices_dict:
                # Just extend it with the current last value.
                self.internal_dict[col].extend([self.indices_dict[col]] *
                                               max_len)

            else:
                # Just extend it with the current last value.
                self.internal_dict[col].extend([self.internal_dict[col][-1]] *
                                               max_len)
    ...
    def save_to_npy(self, file_prefix, data):
        ...
        for index in sorted(self.indices_dict.keys()):
            file_prefix += '_' + index + '-' + str(
                int(self.indices_dict[index]))

        print('Saving a %d-length list of %s-shape elements to %s.' %
              (len(data), str(data[0].shape), file_prefix),
              flush=True)
        stacked_data = np.stack(data)
        np.save(os.path.join(self.data_dir, file_prefix),
                stacked_data,
                allow_pickle=False)
        print('Saved the resulting %s-shape array to %s.' %
              (str(stacked_data.shape), file_prefix),
              flush=True)
...
if __name__ == '__main__':
    data_logger = DataLogger(
        ['iter', 'ppo_perf', 'ppo_lens', 'ppo_rews'], 'test_data.csv',
        'C:/Users/Leonardo/Documents/GitHub/BaRC/data_logged/')
