...
def visualize_starts(starts,
    ...
    if old_starts is not None:
        ...
        if with_arrows:
            plot_arrows(ax,
                        old_xy,
                        old_start_color,
                        alpha=0.5 * old_start_alpha)

        ax.plot(old_xy[:, x_idx],
                old_xy[:, y_idx],
                alpha=old_start_alpha,
                c=old_start_color,
                marker='o',
                markeredgecolor=edgecolor,
                markersize=np.sqrt(size),
                label=old_start_name,
                linestyle='None',
    ...
    if starts is not None and len(starts) > 0:
        ...
        if rewards_map is None:
            ...
            for start in starts:
                ...
                starts_color.append(rewards_map[start_str] if start_str in
                                    rewards_map else 0.0)

        if with_arrows:
            plot_arrows(ax, xy, starts_color)

        sc = ax.scatter(xy[:, x_idx],
                        xy[:, y_idx],
                        c=starts_color,
                        edgecolor=edgecolor,
                        s=size,
                        label='Starts' if old_starts is None else 'New Starts',
                        cmap=cm,
                        vmin=0,
                        vmax=1,
                        linewidths=edgewidths,
                        alpha=0.8)
    ...
    if problem.env_name == 'DrivingOrigin-v0':
        ...
        for xo, yo, ro in zip(problem.env.unwrapped.obst_X,
                              problem.env.unwrapped.obst_Y,
                              problem.env.unwrapped.obst_R):
            c = plt.Circle((xo, yo), ro, color='black', alpha=1.0)
            ax.add_artist(c)

        r = plt.Rectangle(
            (problem.env.unwrapped.xg_lower, problem.env.unwrapped.yg_lower),
    ...
    if problem.env_name == 'DrivingOrigin-v0':
        ...
        ax.set_title(
            r'Goal State: [$x$ = %.2f, $y$ = %.2f, $\theta$ = %.2f, $v$ = %.2f, $\kappa$ = %.2f]'
            % (x, y, th, v, kap) + title_adj)
    elif problem.env_name == 'PlanarQuad-v0':
...
def visualize_rollouts(starts, policy, problem, figfile=None):
    ...
    fig, ax = plt.subplots()
    for start in starts:
        success, traj, actions = rl_utils.rollout(policy,
                                                  start,
                                                  problem,
                                                  return_actions=True)
    ...
    if problem.env_name == 'PlanarQuad-v0':
        for coord in range(traj.shape[1]):
            ...
            fig.savefig(figfile + '_coord_%d.pdf' % coord,
                        dpi=DPI,
                        transparent=True)
            plt.close(fig)
        ...
        for coord in range(actions.shape[1]):
            ...
            fig.savefig(figfile + '_action_coord_%d.pdf' % coord,
                        dpi=DPI,
                        transparent=True)
            plt.close(fig)
