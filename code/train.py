...
from utils import *
from code.curriculum import random, update_backward_reachable_set, sample_from_backward_reachable_set, backward_reachable
from code.problem import Problem
from code.rl import ppo
...
parser.add_argument("type", help="what kind of curriculum to employ", type=str)
parser.add_argument(
    "--debug",
    help="whether to print loads of information and create plots or not",
    action="store_true")
parser.add_argument("--brs_sample",
                    help="how to sample from a backreachable set",
                    type=str,
                    default='contour_edges')
parser.add_argument("--run_name",
                    help="name of run that determines where outputs are saved",
                    type=str,
                    default=None)
parser.add_argument("--zero_goal_v",
                    help="whether to make the goal have zero initial velocity",
                    action="store_true")
parser.add_argument(
    "--finish_threshold",
    help="what fraction of starts must be successful to finish training.",
    type=float,
    default=0.95)
parser.add_argument("--seed",
                    help="what seed to use for the random number generators.",
                    type=int,
                    default=2018)
parser.add_argument("--disturbance",
                    help="what disturbance to use in the gym environment.",
                    type=str,
                    default=None)
parser.add_argument("--gym_env",
                    help="which gym environment to use.",
                    type=str,
                    default='DrivingOrigin-v0')
parser.add_argument("--hover_at_end",
                    help="whether to null velocity and rates at the end",
                    action="store_true")
parser.add_argument("--variation",
                    help="what variation to use",
                    type=int,
                    default=None)
...
if args.type in ['backreach', 'random', 'ppo_only']:
    ...
    parser.error('"%s" is not in ["backreach", "random", "ppo_only"]!' %
                 args.type)

RUN_DIR = os.path.join(
    os.getcwd(), 'runs',
    args.gym_env + '_' + run_dir + '_' + strftime('%d-%b-%Y_%H-%M-%S'))
...
if args.gym_env == 'DrivingOrigin-v0':
    X_IDX = 0
    Y_IDX = 1
    from code.backreach.car5d_interface import Car5DBackreachEngine as BackreachEngine
elif args.gym_env == 'PlanarQuad-v0':
    X_IDX = 0
    Y_IDX = 2
    from code.backreach.quad6d_interface import Quad6DBackreachEngine as BackreachEngine


def train_step(weighted_start_states,
               policy,
               train_algo,
               problem,
               num_ppo_iters=20):
    ...
    return train_algo.train(problem,
                            policy,
                            min_cost=problem.min_cost,
...
def train(
        problem,  # Problem object, describing the task.
        initial_policy,  # Initial policy.
        goal_state,  # Goal state (s^g).
        full_start_dist,  # Full start state distribution (rho_0).
        N_new=200,
        N_old=100,  # Num new and old start states to sample.
        R_min=0.5,
        R_max=1.0,  # Reward bounds defining what a "good" start state is.
    ...
    data_logger = DataLogger(col_names=[
        'overall_iter', 'ppo_iter', 'overall_perf', 'overall_area', 'ppo_perf',
        'ppo_lens', 'ppo_rews'
    ],
                             filepath=os.path.join(DATA_DIR,
                                                   'data_%s.csv' % args.type),
                             data_dir=DATA_DIR)
...
def train_ppo_only(**kwargs):
    ...
    while i < num_iters * num_ppo_iters:
        ...
        data_logger.update_indices({
            "overall_iter": i,
            "ppo_iter": ppo_iter_count
        })
...
def train_pointwise(**kwargs):
    ...
    while i < num_iters:
        ...
        if debug and len(new_starts) > 0:
            visualize_starts(new_starts,
                             problem,
                             figfile=os.path.join(FIGURES_DIR,
                                                  'curric_starts_iter_%d' % i))
        ...
        if debug:
            visualize_starts(new_starts,
                             ...
                             figfile=os.path.join(
                                 FIGURES_DIR,
                                 'replay_and_curric_starts_iter_%d' % i))
            ...
            visualize_starts(None,
                             ...
                             figfile=os.path.join(
                                 FIGURES_DIR, 'previous_starts_iter_%d' % i))
        ...
        if debug:
            if problem.env_name == 'DrivingOrigin-v0':
                visualize_starts(starts,
                                 ...
                                 figfile=os.path.join(
                                     FIGURES_DIR, 'start_rews_iter_%d' % i))

            visualize_rollouts([sample(starts)],
                               pi_i,
                               problem,
                               figfile=os.path.join(FIGURES_DIR,
                                                    'rollouts_iter_%d' % i))
...
if __name__ == '__main__':
    ...
    with fixed_random_seed(args.seed):
        ...
        if args.type == 'random':
            ...
            raise ValueError("%s is an unknown curriculum strategy!" %
                             args.type)
