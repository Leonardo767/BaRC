from code.utils import uniform, sample
...
def update_backward_reachable_set(starts, **kwargs):
    ...
    if variation == 1 and br_engine.check_membership(
            np.array([problem.env.unwrapped.start_state])):
        print('Variation 1 condition curriculum.py!', flush=True)
        return

    br_engine.update_and_compute_backward_reachable_set(
        starts,
        plot=kwargs['debug'],
...
def sample_from_backward_reachable_set(N_new, **kwargs):
    ...
    if variation == 2 and br_engine.check_membership(
            np.array([problem.env.unwrapped.start_state])):
        print('Variation 2 condition curriculum.py!', flush=True)
        return [problem.env.unwrapped.start_state]

    new_starts_arr = br_engine.sample_from_grid(size=N_new,
                                                method=kwargs['brs_sample'])
